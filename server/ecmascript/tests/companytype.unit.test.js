...
describe('Companytype Unit Tests', function () {
    ...
    describe('Test getCompanytypeById(id)', function () {
        it('getCompanytypeById(1) returns {"id": 1, "name": "Kunde"}', async () => {
            await expect(getCompanytypeById(1)).resolves.toEqual(expect.any(Companytype) && expect.objectContaining({ dataValues: { "id": 1, "name": "Kunde", updatedAt: expect.any(Date), createdAt: expect.any(Date) } }));
        });
    ...
    describe('Test putCompanytypeById(id)', function () {
        ...
        it('getCompanyById(1) returns {"id": 1, "name": "Spediteur"} after Name Change', async () => {
            await expect(getCompanytypeById(1)).resolves.toEqual(expect.any(Companytype) && expect.objectContaining({ dataValues: { "id": 1, "name": "Spediteur", updatedAt: expect.any(Date), createdAt: expect.any(Date) } }));
        });
