...
import { CompanyResponse } from '../controllers/companies/index.js'


function update_company(company: CompanyResponse) {
...
export const addCompany = (body: CompanyResponse) => new Promise<Company>(async function (resolve, reject) {
    const company: CompanyResponse = { name: body.name }
    if (body.abbr) {
...
export const putCompanyById = (id: number, body: CompanyResponse) => new Promise<void>(async (resolve, reject) => {
    try {
        const company = await Company.findByPk(id)
        if (company === null) {
            reject(new NotFoundError())
